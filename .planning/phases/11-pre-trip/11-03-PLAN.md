...
export function PassportUpload({ bookingId, existingDocument, onUploadComplete }: PassportUploadProps) {
  ...
  const handleUpload = async (useCamera: boolean) => {
    ...
    try {
      ...
      // For now, use the local URI (will be replaced with actual upload)
...
Also update mobile/app/(app)/(tabs)/index.tsx (dashboard) to link to trip detail:
- When user taps a booking card, navigate to `/trip/${tripId}` where tripId comes from the booking's trip relation.
  </action>
  <verify>
TypeScript compiles: `cd mobile && npx tsc --noEmit`
...
3. Navigation works: Expo Router recognizes trip/[tripId] routes
4. Countdown updates: Hook recalculates on foreground
</verification>

<success_criteria>
- [ ] useCountdown hook with AppState awareness
- [ ] CountdownTimer shows days/hours/minutes with brand colors
- [ ] PassportUpload handles camera and gallery
